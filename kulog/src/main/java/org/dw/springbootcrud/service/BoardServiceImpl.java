...
*/
    ...
    public PageResponseDTO<BoardDTO> getBoardListByWriter(String uuid, PageRequestDTO pageRequestDTO){
        ...
        Page<Board> result = boardRepository.getAllByWriter_uuid(uuid, pageable);
    ...
    public PageResponseDTO<BoardDTO> getMyBoardList(String uuid, PageRequestDTO pageRequestDTO){
        ...
        Page<Board> results = boardRepository.getAllByWriter_uuid(uuid, pageable);

        List<BoardDTO> dtoList = results.getContent().stream()
                .map(board ->{
                    int replyCount = commentRepository.countByBoard(board);
                    Long likeCount = boardLikesRepository.countByBoard(board);
                    List<UploadFile> uploadFiles = uploadFileRepository.findByBoardOrderByFileOrderAsc(board);
