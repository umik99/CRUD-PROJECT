...
public class FileServiceImpl implements FileService {
    ...
    private static final String PROFILE_DIR = BASE_DIR + File.separator + "profiles/";
    ...
    public String saveProfileImage(MultipartFile file, User user) throws IOException {
        ...
        File originalFile = new File(profileDir, savedName);

        Thumbnails.of(file.getInputStream()).size(120, 120).toFile(originalFile);
    ...
    public void deleteProfileImage(String file) {
        ...
        File target = new File(PROFILE_DIR + file);
        if (target.exists()) {
            target.delete();
        }
    }
